...
export function assertVisibility(value) {
  ...
  if (!VISIBILITIES.includes(normalized))
    throw new ApiAuthError(400, "不支持的可见范围");
  return normalized;
}
...
export async function requirePerson(prisma, tenantId, personId) {
  const person = await prisma.familyData.findUnique({
    where: {
      tenant_id_person_id: { tenant_id: tenantId, person_id: String(personId) },
    },
  });
...
export function canReadPrivate(record, userId) {
  return (
    ...
    Number(record.created_by) === Number(userId)
  );
}
...
export function serializeMemory(memory, { includeDrafts = false } = {}) {
  ...
  return {
    ...
    transcriptDetails: includeDrafts
      ? asJson(memory.transcript_details, null)
      : undefined,
...
export async function createAuditLog(
  prisma,
  { tenantId, actorUserId, action, entityType, entityId, metadata },
) {
