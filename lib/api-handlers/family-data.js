  ...
  try {
    if (req.method === "GET") {
      if (tenantId === DEFAULT_TENANT_ID) {
        const { default: familyData } = await import("../../src/data/familyData.js");
        return res.json({
          success: true,
          data: familyData,
      ...
      const [tenant, records, version] = await Promise.all([
      ...
      const data = records
        .map(toClientPerson)
        .map((person) => redactLivingPerson(person, membership.role, tenant?.settings));
      return res.json({
        success: true,
        data,
        count: data.length,
