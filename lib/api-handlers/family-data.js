...
const familyData = createRequire(import.meta.url)("./data/familyData.json");
...
export default async function handler(req, res) {
  ...
  const tenantId =
    req.headers["x-tenant-id"] || req.body?.tenantId || DEFAULT_TENANT_ID;
  ...
  try {
    if (req.method === "GET") {
      ...
      const data = records
        .map(toClientPerson)
        .map((person) => redactLivingPerson(person, membership.role));
      return res.json({
        success: true,
        data,
        count: data.length,
    ...
    if (req.method === "POST") {
      ...
      if (!Array.isArray(submittedData))
        return res
          .status(400)
          .json({ success: false, error: "家谱数据格式错误" });
      ...
      return res.json({
        success: true,
        message: "家谱数据保存成功",
        savedCount: submittedData.length,
        version,
      });
    }
