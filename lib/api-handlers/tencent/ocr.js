...
import { recognizeImage } from "../../cloud/ocr.js";
import { requireStoryWrite } from "../../personStories.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!Array.isArray(imageUrls) || !imageUrls.length || imageUrls.length > 10)
      return res
        .status(400)
        .json({ success: false, error: "请上传 1 至 10 张家谱图片" });
    ...
    for (const imageUrl of imageUrls) {
      ...
      const parsed = await parseFamilyTreeImage({
        imageUrl,
        ocrText: ocr.text,
      });
      await recordCloudUsage(prisma, {
        ...
        unit: "token",
        metadata: { model: "vision" },
      });
      const candidates = Array.isArray(parsed.data?.people)
        ? parsed.data.people
        : [];
      const offset = people.length;
      people.push(
        ...candidates.map((item, index) =>
          normalizePerson(item, offset + index),
        ),
      );
    ...
    return res.json({
      ...
      count: people.length,
      jobId: job.id,
      rawOcr: rawResults.map((item) => item.ocr),
    });
  } catch (error) {
