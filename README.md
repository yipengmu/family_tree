...
一个公益性的多账号数字家谱：让普通家庭可以看家谱、续家谱、管家谱。
...
- 游客只读浏览“穆氏示范家谱”。
- 手机号验证码注册（注册时设置个人姓名和密码）、手机号或邮箱密码登录、密码找回与重置、JWT 会话和个人资料读取。
- 新用户可从示范家谱进入注册；注册姓名会作为账户显示名和家谱发起人的默认姓名，空家谱使用两步向导先确认本人，再补充父亲或一位长辈，保存后直接查看第一份家谱。
...
- 人物志可在浏览器本地生成朋友圈长图；家谱 Owner 还可以显式发布一份固定 7 天有效的在线家谱快照。发布成功后分享人直接进入与查看者相同的公开访问页，不再维护一套本地仿制预览；在线页展示真实姓名、代际、树状关系、聚合摘要和家谱文化内容，但不包含照片、精确生日、地域、住址、联系方式、证件、人物志或原始材料。Owner 可提前撤销或更新后重新生成链接。
...
管理员数据分析后台位于 `/bi`。页面和 `/api/admin/analytics` 接口都要求当前 JWT 用户邮箱命中服务端 `ADMIN_EMAILS` 白名单；数据库统计默认排除 `default,user_1`（穆氏示范/迁移家谱）。后台包含逐份家谱的创建者最小账号标识、匿名世代结构缩略图、人物/关系/故事/素材/版本明细，以及严格顺序漏斗、环节耗时、失败后快速流失、终端分布、访问时段和 D1/D7/D30 留存。若要显示行为指标，还需要在服务端配置 `POSTHOG_API_KEY`、`POSTHOG_PROJECT_ID` 和 `POSTHOG_API_HOST`，这些变量不能使用 `REACT_APP_` 前缀。
...
| GET/POST         | `/api/shares`                     | 查看或发布当前家谱的 7 天分享  |
| DELETE           | `/api/shares/:shareId`            | Owner 提前撤销在线分享         |
| GET              | `/api/public-shares/:token`       | 免登录读取限时公开家谱快照     |
...
### 8. 在线分享已形成首版成果物，长期回访闭环仍不完整

用户现在可以发布一份 7 天有效、可撤销的只读家谱网页，亲友免登录查看真实姓名与树状关系，并从分享页进入注册建谱。人物志公开成果、邀请长辈补充、家族年鉴和纪念页仍需继续形成长期回访闭环。

## 建议优先级

1. 观察 membership 迁移和存量账号登录/建谱链路，补齐迁移后的运营检查。
2. 本地 Express 已收敛为与 Vercel 共用 API handler 的传输壳，后续只新增 `api/` 与 `lib/api-handlers/` 业务实现。
3. 观察 7 天在线家谱分享的访问、创建点击和首条关系转化，再完成邀请、成员管理和补充确认。
