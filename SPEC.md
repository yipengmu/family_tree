...
- 人物志支持在浏览器本地生成分享长图。家谱 Owner 还可显式发布固定 7 天有效的在线家谱快照：发布成功后分享人和查看人进入同一个公开访问路由、读取同一份服务端快照，不维护独立的本地分享预览；公开真实姓名、性别、代际和亲属关系，不因人物在世或未成年自动隐藏姓名；照片、精确生日、地域、住址、联系方式、证件、人物志和原始材料始终不进入公开快照。Owner 可提前撤销或更新内容并生成新链接。
...
分享分为人物志本地静态长图和 Owner 主动发布的 7 天在线家谱，两者权限与内容边界分开：
...
- 人物志长图漏斗记录 `share_poster_generated`、`share_poster_saved`，仅包含 `person` 类型和保存方式，不采集姓名、家谱名称、纪事原文、图片或二维码内容。

在线家谱分享执行以下契约：

- 只有当前租户 `OWNER` 可以发布、更新或撤销；发布前必须预览将公开的全部人物姓名，并确认链接可能被转发或截图。
- 分享入口只负责读取现有链接或完成发布前确认；已有有效链接时直接进入 `/s/:token`，新链接发布成功后也立即进入该路由。分享人和查看人必须由同一个公开页组件读取同一份匿名快照，禁止再维护内容、样式或交互不同的 Owner 本地预览页。
...
- 每份链接从发布时起固定有效 `7 × 24` 小时；公开页显著提示 7 天有效。到期后匿名 API 不再返回快照，页面转为产品说明和创建入口。
