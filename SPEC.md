...
- 桌面端的“免费创建”优先引导手机继续，同时保留“在电脑上开始”；移动端直接进入 H5 创建链路。
...
人物生平是下一阶段产品里程碑，官网必须标记为 `正在共创`，不能伪装成已经完整上线的能力。
...
官网与 H5 产品使用统一 URL 语义，设备只改变版式，不改变页面身份：
...
- 当前 `/` 直接进入家谱应用的行为需要在官网实现时迁移到 `/app`；迁移不能破坏登录、注册、重置密码、租户选择和现有 H5 内部导航。
- 官网主按钮在移动端直接进入 `/app` 的创建链路；桌面端优先展示手机二维码，同时提供“在电脑上开始”。
- 示范谱、邀请、人物分享等传播场景应使用对应深链接，不强迫用户绕回官网。
- 移动官网保持短路径：首屏可直接开始、底部可提供不遮挡内容的固定行动按钮；不能通过设备判断自动把 `/` 重定向到 `/app`。
...
1. `/` 在桌面和手机上都展示官网，`/app` 承载现有 H5，`/app/demo` 可直接浏览只读穆氏示范家谱。
2. 官网无需等待认证、租户或家谱数据请求即可显示完整首屏。
3. 首屏使用本规格确定的价值层级，并同时提供创建和示范谱入口。
4. 移动端主行动可以在一次点击内进入 H5 创建链路；桌面端可以扫码到手机，也可以继续在电脑上开始。
...
- `/` 产品官网、`/app` H5 产品和 `/app/demo` 穆氏只读示范谱已经拆分；官网包含双线建谱主张、真实故事、隐私与开源、公众号共创入口以及桌面扫码/移动直达创建链路。
...
在第一版家谱已经形成之后，提供脱敏分享、成员邀请、角色权限、待确认、评论、来源、时间线、照片/口述史管理。
