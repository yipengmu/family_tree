...
| `/login`      | 手机号验证码登录/注册，并保留邮箱密码兼容入口       |
| `/register`   | 兼容入口；新用户默认引导至手机号登录/注册           |
...
- 手机号认证方案已确定：PMF 阶段采用阿里云号码认证服务的短信认证 API；手机号验证码验证成功后，未注册手机号自动创建账号和私密家谱空间，姓名延后到“录入本人”流程填写。
...
手机号认证是 PMF 阶段的新用户默认入口，采用阿里云号码认证服务的短信认证 API（服务端调用 `SendSmsVerifyCode`/`CheckSmsVerifyCode` 能力）。服务端必须配置阿里云控制台当前可用的系统签名、登录/注册模板和对应方案名称；本阶段不接入运营商本机号码一键登录，也不依赖原生 App SDK。

- `/api/auth/phone/send-code` 接收规范化的中国大陆手机号和用途；服务端调用阿里云发送验证码 API，并执行手机号、IP、设备和每日预算限流。
- `/api/auth/phone/verify` 调用阿里云核验验证码；验证成功后按手机号身份查找用户。已存在用户直接登录；不存在则在同一事务内创建用户、私密 Tenant、OWNER membership 并签发 JWT。
- 新用户不在认证页填写姓名和密码；验证成功后进入 `/app/create`，在“录入本人”步骤填写姓名并保存第一位人物。账号显示名可以在此时同步更新。
- 新用户首次认证成功后，必须保留 `returnTo`，不能因自动注册丢失原来的创建入口或邀请深链。
...
- 手机号使用服务端密钥 HMAC 作为唯一检索值，默认只保存掩码或加密值；日志、分析事件和错误消息不得包含完整手机号。
...
- 记录 `phone_code_requested`、`phone_code_sent`、`phone_code_verified`、`phone_auth_completed`，事件属性只包含脱敏用户 ID、设备、结果和供应商错误类别。
...
采用阿里云短信认证 API，完成手机号验证码登录/自动注册、最长 60 天可撤销会话、姓名后置到“录入本人”、邮箱存量账号兼容、手机号绑定、邀请兼容、防盗刷限流、隐私同意、审计和认证漏斗指标。未完成前不得对外承诺手机号登录已上线。
...
图片解析人工审核、重复人物提示、故事整理、家族年鉴/PDF/分享页生成。
