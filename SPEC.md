...
管理员后台 `/admin/analytics` 用于观察产品业务健康度，不替代面向用户的产品北极星定义：
...
- DAU、漏斗和 D1/D7/D30 留存使用脱敏后的 PostHog 用户 ID 与核心业务事件计算；缺少服务端查询配置时必须明确展示数据未连接，不得以 0 伪造结果。
- 报表只返回聚合数据，不返回姓名、邮箱、联系方式、住址、家谱内容或人物级隐私字段。穆氏示范/迁移租户通过服务端 `ANALYTICS_EXCLUDED_TENANT_IDS` 排除。
...
- 手机号认证采用阿里云号码认证服务的短信认证 API；验证码只进入注册和密码找回等身份验证流程，不进入正常登录。注册时必须同步设置密码，并原子创建账号、私密家谱空间和 OWNER membership；姓名延后到“录入本人”流程填写。
...
- 私密家谱主界面和人物志支持在浏览器本地生成分享长图。家谱长图默认隐藏在世及生存状态待确认人物姓名；人物志长图默认不展示在世人物的照片、出生日期和地域，且只纳入已发布、非 `PRIVATE` 的纪事。图片中的二维码只指向谱里产品入口，不创建免登录家谱访问权限。
...
- 可撤销、可审计的在线公开分享页仍未实现；当前只有用户主动生成并保存的静态分享长图。媒体元数据管理、结构化导入导出和家族年鉴成果物仍不完整。
...
- 新用户在认证页设置密码，但不填写姓名；注册成功后进入 `/app/create`，在“录入本人”步骤填写姓名并保存第一位人物。账号显示名可以在此时同步更新。
...
首版分享是用户在已授权会话中主动生成的静态成果物，不是把私密家谱变成公开网页：
...
后续在线公开成果页必须另行设计显式发布、字段级预览、不可猜测的范围令牌、过期时间、Owner 撤销、访问审计和服务端隐私裁剪；不能把当前静态图片的产品选择静默升级为永久公开链接。
