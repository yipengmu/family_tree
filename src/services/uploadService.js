...
class UploadService {
  ...
  async uploadFiles(files, tenantId, options = {}) {
    ...
    for (let index = 0; index < files.length; index += 1) {
      ...
      if (!signedResponse.ok || !signed.success)
        throw new Error(signed.error || "无法获取安全上传地址");
  ...
  async deleteOSSFile() {
    throw new Error("请通过媒体管理页删除图片");
  }
  async getOSSFileInfo() {
    throw new Error("浏览器不再直接读取 OSS 元数据");
  }
  generateTempUrl() {
    throw new Error("临时访问地址需要由服务端生成");
  }
}
