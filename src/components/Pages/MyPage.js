...
const MyPage = ({ activeMenuItem = 'mine', onMenuClick, familyData = [] }) => {
  ...
  const exportFamilyData = () => {
    ...
    const blob = new Blob([JSON.stringify({ tenant, familyData, exportedAt: new Date().toISOString() }, null, 2)], {
    ...
    <AppLayout activeMenuItem={activeMenuItem} onMenuClick={onMenuClick}>
      <div className="my-page">
        <header className="my-page-header">
          ...
          <div>
            <p>我的谱里</p>
            <h1>{authenticated ? (user?.name || user?.email || '家谱整理者') : '把家人的名字留在一起'}</h1>
          </div>
        </header>

        {!authenticated ? (
          <section className="my-guest-card">
            <span className="my-card-kicker">家谱默认私密</span>
            <h2>创建一份只属于家人的数字家谱</h2>
            <p>从自己开始记录，慢慢补齐父母与祖辈。只有你邀请的人才能查看。</p>
          ...
          <>
            <section className="my-family-card">
              <div className="my-family-card-top">
                ...
                <span className="my-private-pill"><LockOutlined /> 私密</span>
              </div>
              <div className="my-family-stats">
                <div><strong>{familyData.length}</strong><span>位族人</span></div>
                <div><strong>{new Set(familyData.map((person) => person.g_rank).filter(Boolean)).size || 0}</strong><span>代相承</span></div>
              </div>
              <TenantSelector />
            </section>
            ...
            <section className="my-menu-card" aria-label="家谱管理">
              ...
              <button type="button" onClick={() => message.info('家谱默认私密，在世人物信息受保护')}>
                <span className="my-menu-icon"><SafetyCertificateOutlined /></span>
                <span><strong>隐私与安全</strong><small>了解谁可以看到哪些资料</small></span>
                <RightOutlined />
              </button>
            </section>

            <button type="button" className="my-logout-button" onClick={logout}>退出登录</button>
          </>
        )}
